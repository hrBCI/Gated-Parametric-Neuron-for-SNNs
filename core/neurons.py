...
class Cuba_LIF(MemoryModule):
    def __init__(self, alpha: float = 0.5, beta: float = 0.5, v_threshold: float = 1., v_reset: float = 0., input_channels:int = 128, 
                 surrogate_function: Callable = surrogate.ATan()):
...
class GPN(MemoryModule):
    def __init__(self, input_channels: int = 128, surrogate_function: Callable = surrogate.ATan()):
...
class GPN_wo_FI(MemoryModule):
    def __init__(self, input_channels: int=128, surrogate_function: Callable = surrogate.ATan()):
        ...
        self.ln = nn.Linear(in_features=2*self.input_channels,
                              out_features=2*self.input_channels
                             )
    ...
    def forward(self, x: torch.Tensor):
        ...
        cc_b, cc_t = torch.split(combined_ln, self.input_channels, dim=1)
        
        b = torch.tanh(cc_b)
        t = torch.sigmoid(cc_t)
   
        v_hidden = (1-self.leaky) * self.v + self.leaky * x
...
class GPN_wo_I(MemoryModule):
    def __init__(self, input_channels: int=128, surrogate_function: Callable = surrogate.ATan()):
        ...
        self.ln = nn.Linear(in_features=2*self.input_channels,
                              out_features=3*self.input_channels
                             )
    ...
    def forward(self, x: torch.Tensor):
        ...
        cc_f, cc_b, cc_t = torch.split(combined_ln, self.input_channels, dim=1)
        
        f = torch.sigmoid(cc_f)
        b = torch.tanh(cc_b)
        t = torch.sigmoid(cc_t)
   
        v_hidden = f * self.v + (1-f) * x
...
class GPN_wo_T(MemoryModule):
    ...
    def forward(self, x: torch.Tensor):
        ...
        cc_f, cc_i, cc_b = torch.split(combined_ln, self.input_channels, dim=1)
        ...
        self.spike = self.surrogate_function(v_hidden - self.threshold)
...
class GPN_wo_B(MemoryModule):
    ...
    def forward(self, x: torch.Tensor):
        ...
        cc_f, cc_i, cc_t = torch.split(combined_ln, self.input_channels, dim=1)
        ...
        spike_reset = self.spike.detach()
        self.v = (1. - spike_reset) * v_hidden + spike_reset * self.v_reset

        return self.spike


class GPN_wo_FITB(MemoryModule):
    def __init__(self, input_channels: int=128, surrogate_function: Callable = surrogate.ATan()):
