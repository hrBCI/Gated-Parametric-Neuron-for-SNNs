...
def Parser():
    ...
    parser = argparse.ArgumentParser(description='based on spikingjelly')
    ...
    parser.add_argument('-T', default=50, type=int, help='simulating time-steps')
...
def main_max(args):
    ...
    train_dataset,val_dataset,test_dataset = tools.F_audio_datasets(args.data_name,args.data_path,args.T,args.aug)  
    ...
    val_loader = DataLoader(val_dataset, batch_size=args.batch_size, shuffle=True,drop_last=False,num_workers=1,pin_memory=True)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for frame, label in train_loader:
            ...
            loss = loss_func(out_rec_TNO,label,35)
        ...
        print("epoch:%d  loss:%.4f  train_acc:%.3f  test_acc:%.3f  max_acc_epoch:%.3f/%d  time:%.1fs"%(epoch,train_loss,100*train_acc,100*test_acc,100*max_test_acc,max_test_epoch,time.time()-start_time))
...
if __name__ == '__main__':
    ...
    tools.F_init_seed(2024)
    acc_max=(main_max(args))
    
    print('\n\n\n=========================')
    print('max test acc:%.2f'%(acc_max))
